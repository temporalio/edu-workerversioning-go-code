...
1. Edit the `worker/main.go` file. This file contains a Temporal Worker that
   ...
   `worker.Options{}`. Of these, `UseVersioning: true` should always be enabled
...
2. For this Exercise, you do not need to edit `DeploymentName:`, which has been
   ...
   `workflow.VersioningBehaviorPinned` or
   `workflow.VersioningBehaviorAutoUpgrade`. This will determine whether new
   ...
   manually. For now, set it to `workflow.VersioningBehaviorPinned`.
3. Start your Worker by running `go run worker/main.go`. You should receive
...
4. In another terminal, run `temporal worker deployment describe --name="worker_versioning_demo"` to get some metadata returned from your running Worker. This should also return your Build ID:
   ...
   ```
5. Next, kick off a Workflow by running `go run start/main.go a100`. You can
...
1. Now, you'll make some changes to your Workflow code and increment your Worker
   version. Edit the `workflow.go` file. You should find two copies of the
   `workflow.ExecuteActivity(ctx, SendThankYouToCustomer, input)` code block
...
2. Edit the `worker/main.go` file again, and increment your BuildID to `2.0`.
   Save the file. In another terminal, start another Worker by running `go run
   worker/main.go`. You should now have both a "1.0" and a "2.0" Worker running.
...
4. Now, if you were to kick off another Workflow, for example by running
   `go run start/main.go a101`, that Workflow would again be handled by the
...
5. Finally, after running this command, go ahead and run
   `go run start/main.go a101` to kick off a new Workflow. It will be handled by
